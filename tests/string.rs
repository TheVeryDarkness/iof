use iof::{locale::ASCII, show, unwrap, InputStream, ReadFrom, ReadInto, ReadOneFrom, ReadOneInto};
use std::io::Cursor;

#[allow(dead_code)]
mod check_impl {
    use iof::dimension::Dimension;
    const fn check_impl<T: Dimension + ?Sized>() {}
...
fn read_strings() {
    ...
    assert!(<String>::try_read_from(&mut reader, &ASCII).is_err());
}
...
fn read_in_line() {
    ...
    assert!(<String>::try_read_some_in_line_from(&mut reader, &ASCII).is_err());
}
...
fn read_in_line_trimmed_unicode() {
    ...
    assert!(<String>::try_read_in_line_trimmed_from(&mut reader, &ASCII).is_err());
}
...
fn read_in_line_some_unicode() {
    ...
    assert!(<String>::try_read_in_line_some_trimmed_from(&mut reader, &ASCII).is_err());
}
