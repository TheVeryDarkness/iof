...
fn read_array_0() {
    ...
    assert!(<[u32; 0]>::try_read_from(&mut reader, Default::new()).is_ok());
    assert!(<[u32; 1]>::try_read_from(&mut reader, Default::new()).is_err());
}
...
fn read_boxed_array_0() {
    ...
    assert!(<Box<[u32; 0]>>::try_read_from(&mut reader, Default::new()).is_ok());
    assert!(<Box<[u32; 1]>>::try_read_from(&mut reader, Default::new()).is_err());
}
