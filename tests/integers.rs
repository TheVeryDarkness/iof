...
fn try_read_single_3() {
    ...
    assert!(<u32>::try_read_one_from(&mut reader, &ASCII).is_err());
}
...
fn read_char_3() {
    ...
    assert!(<u32>::try_read_in_char_from(&mut reader, &ASCII).is_err());
}
...
fn read_one_then_all_in_line_some() {
    ...
    assert!(<u32>::try_read_some_in_line_from(&mut reader, &ASCII).is_err());
}
#[test]
fn read_all() {
